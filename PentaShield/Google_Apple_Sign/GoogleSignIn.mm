...
// ============================================
// [개선 사항] GoogleSignIn 7.0+ API 마이그레이션
// ============================================

@end

/**
 * Helper method to handle sign-in completion
 */

// ============================================
// [개선 사항] 에러 코드 매핑 개선
// 개선: GoogleSignIn 7.0+의 새로운 에러 코드를 세분화하여 매핑
// 효과: 취소, 키체인 오류 등 구체적인 에러 타입 구분 가능
// ============================================
void HandleSignInCompletion(GIDGoogleUser *user, NSError *_error) {
  if (_error == nil) {
    if (currentResult_) {
    ...
    if (currentResult_) {
      // GoogleSignIn 7.0+ uses different error codes
      // Map errors based on the domain and code
      if ([_error.domain isEqualToString:@"com.google.GIDSignIn"]) {
        switch (_error.code) {
        case -1: // Canceled
          currentResult_->result_code = kStatusCodeCanceled;
          break;
        case -2: // Has no auth in keychain
...
// Store configuration for use in sign-in
static NSString *configuredClientID = nil;
static NSString *configuredServerClientID = nil;
static NSMutableArray *configuredScopes = nil;
static NSString *configuredLoginHint = nil;
...
bool GoogleSignIn_Configure(void *unused, bool useGameSignIn,
  ...
  // Read clientID from GoogleService-Info.plist if not provided
  if (!configuredClientID) {
    NSString *path = [[NSBundle mainBundle] pathForResource:@"GoogleService-Info"
                                                     ofType:@"plist"];
    NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];
    configuredClientID = [dict objectForKey:@"CLIENT_ID"];
  }

  if (webClientId) {
    configuredServerClientID = [NSString stringWithUTF8String:webClientId];
  }

  int scopeSize = scopeCount;

  if (scopeSize) {
    configuredScopes = [[NSMutableArray alloc] initWithCapacity:scopeSize];
    for (int i = 0; i < scopeCount; i++) {
      [configuredScopes addObject:[NSString stringWithUTF8String:additionalScopes[i]]];
    }
  }

  if (accountName) {
    configuredLoginHint = [NSString stringWithUTF8String:accountName];
  }
...
// ============================================
// [개선 사항 ] GIDConfiguration 도입
// 개선: GIDConfiguration 객체를 사용하여 설정 관리
// 효과: 요청별 설정 가능, SDK 7.0+ 권장 방식 준수
// ============================================
void *GoogleSignIn_SignIn() {
  ...
  if (!result) {
    ...
    GIDConfiguration *config = [[GIDConfiguration alloc] initWithClientID:configuredClientID
                                                         serverClientID:configuredServerClientID];
    [GIDSignIn sharedInstance].configuration = config;

    // GoogleSignIn 7.0+ API
    if (configuredScopes && configuredScopes.count > 0) {
      [[GIDSignIn sharedInstance] signInWithPresentingViewController:UnityGetGLViewController()
                                                                 hint:configuredLoginHint
                                                     additionalScopes:configuredScopes
                                                           completion:^(GIDSignInResult *signInResult, NSError *error) {
        HandleSignInCompletion(signInResult.user, error);
        UnityPause(false);
      }];
    } else {
      [[GIDSignIn sharedInstance] signInWithPresentingViewController:UnityGetGLViewController()
...
void *GoogleSignIn_SignInSilently() {
  ...
  if (!result) {
    // ============================================
    // [개선 사항 ] Silent Sign-In API 업데이트
    // ============================================
    // 기존: signInSilently() 메서드 (deprecated)
    // 개선: restorePreviousSignInWithCompletion: 사용
    // 효과: 최신 API 사용, Completion Handler로 결과 처리 일관화
    [[GIDSignIn sharedInstance] restorePreviousSignInWithCompletion:^(GIDGoogleUser *user, NSError *error) {
      HandleSignInCompletion(user, error);
    }];
    result = currentResult_.get();
  }
  return result;
...
size_t GoogleSignIn_GetServerAuthCode(GIDGoogleUser *guser, char *buf,
                                      size_t len) {
  // GoogleSignIn 7.0+ changed serverAuthCode handling
  // It's now only available if requested during configuration
  // For now, we'll return the serverAuthCode if available via private API
  // or empty string if not configured
  NSString *val = nil;
  if ([guser respondsToSelector:@selector(serverAuthCode)]) {
    val = [guser performSelector:@selector(serverAuthCode)];
  }
